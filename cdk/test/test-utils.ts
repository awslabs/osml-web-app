...
/**
 * Fabricated cross-stack values used by Nag_Test_Blocks that would otherwise
 * read dynamic properties from sibling stacks at synthesis time. Exposing
 * these as a shared constant keeps fabricated test inputs in one place and
 * ensures every test uses the same synthetic URLs, bucket names, and stream
 * names.
 */
export const TEST_FABRICATED = {
  mcpUrl: "https://test-stac-loader.example.com/mcp",
  webAppUtilityUrl: "https://test-webapp-utility.example.com",
  modelRunnerApiUrl: "https://test-model-runner.example.com",
  detectionBridgeBucketName: "test-detection-bridge-bucket",
  stacLoaderAlbDnsName: "test-stac-loader-alb.us-west-2.elb.amazonaws.com",
  kinesisStreamName: "test-mr-stream-123456789012"
};
...
export function createTestDeploymentConfig(
  ...
  return {
    ...
    dataplaneConfig: {
      authConfig: {
        authority: "https://test-auth.example.com/realms/test",
        audience: "test-audience",
        clientId: "test-client",
        ...overrides?.dataplaneConfig?.authConfig
      },
      webAppConfig: {
      ...
      modelRunnerApiConfig: {
        ...overrides?.dataplaneConfig?.modelRunnerApiConfig
      },
      webAppUtilityConfig: {
        restrictBucketAccess: true,
        allowedBucketArns: [],
        ...overrides?.dataplaneConfig?.webAppUtilityConfig
      },
      stacLoaderConfig: {
        retentionDays: 30,
        ...overrides?.dataplaneConfig?.stacLoaderConfig
      },
      TILE_SERVER_URL: "https://test-tile-server.example.com",
      STAC_CATALOG_URL: "https://test-stac-catalog.example.com",
      MODEL_RUNNER_QUEUE_ARN:
...
function writeTempSuppressionsFile(
  ...
  try {
    ...
    process.stderr.write(
      `Failed to write temporary suppressions file: ${String(error)}\n`
    );
  }
}
...
/**
 * Writes a suppressed violations report to disk using the supplied
 * in-memory map of stack-name to violations, without reading from or
 * writing to the shared Jest-process-wide temp file. Useful for unit
 * and property tests that need to exercise the report-generation
 * code path in isolation from the running aggregation state of the
 * cdk-nag test suites.
 *
 * @param violationsByStack - Map of stack names to their suppressed violations
 * @param outputPath - Path to the output file
 */
export function writeSuppressedViolationsReportWithData(
  violationsByStack: Map<string, SuppressedNagViolation[]>,
  outputPath: string
): void {
  const allViolations: SuppressedNagViolation[] = [];
  ...
  process.stdout.write(
    `\nSuppressed violations report written to: ${outputPath}\n`
  );
}
