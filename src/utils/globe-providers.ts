...
import { ImageryProviderData } from "@/types/globe.ts";

export interface TerrainProviderData {
  name: string;
  iconUrl: string;
  tooltip: string;
  url?: string;
...
const terrainData: TerrainProviderData[] = [
  {
...
function generateTerrainProviderViewModel(
  data: TerrainProviderData
): ProviderViewModel {
  return new ProviderViewModel({
    name: data.name,
    iconUrl: data.iconUrl,
    ...
    creationFunction: function () {
      ...
      return CesiumTerrainProvider.fromUrl(data.url!);
    }
  });
}

async function generateTerrainProvider(
  data: TerrainProviderData
  ...
  return await CesiumTerrainProvider.fromUrl(data.url!);
}

export async function generateTerrainProviders(): Promise<TerrainProvider[]> {
  return await Promise.all(terrainData.map(generateTerrainProvider));
}

export function generateTerrainProviderViewModels(): ProviderViewModel[] {
  return terrainData.map(generateTerrainProviderViewModel);
}
