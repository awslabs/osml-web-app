...
describe("BedrockService - Property-Based Tests", () => {
  ...
  describe("calculateEffectiveTokens - Property Tests", () => {
    /**
     * Feature: bedrock-model-simplification, Property 3: Burndown Rate Calculation
     * Validates: Requirements 3.1, 3.2, 5.3
     *
     * Property: For any input and output token counts, the effective tokens
     * consumed against quota should equal inputTokens + (outputTokens * 5)
     */
    it("should calculate effective tokens with 5x burndown rate for output tokens", () => {
      fc.assert(
        fc.property(
          fc.nat({ max: 1000000 }), // inputTokens
          fc.nat({ max: 1000000 }), // outputTokens
          (inputTokens, outputTokens) => {
            const { calculateEffectiveTokens, OUTPUT_BURNDOWN_RATE } =
              require("@/services/bedrock-service") as typeof import("@/services/bedrock-service");
            ...
            // Property: effective tokens = input + (output * 5)
            const expected = inputTokens + outputTokens * OUTPUT_BURNDOWN_RATE;

            expect(effectiveTokens).toBe(expected);
            expect(OUTPUT_BURNDOWN_RATE).toBe(5);
          }
        ),
        { numRuns: 100 }
      );
     ...
     * Validates: Requirements 3.1, 3.2, 5.3
     *
     * Property: Effective tokens should always be >= input tokens
     * (since output tokens have positive burndown rate)
     */
    it("should always return effective tokens >= input tokens", () => {
      fc.assert(
        fc.property(
          fc.nat({ max: 1000000 }), // inputTokens
          fc.nat({ max: 1000000 }), // outputTokens
          (inputTokens, outputTokens) => {
            const { calculateEffectiveTokens } =
              require("@/services/bedrock-service") as typeof import("@/services/bedrock-service");
            ...
            // Property: effective tokens >= input tokens (since burndown >= 1)
            expect(effectiveTokens).toBeGreaterThanOrEqual(inputTokens);
          }
        ),
        { numRuns: 100 }
      );
     ...
     * Validates: Requirements 3.1, 3.2, 5.3
     *
     * Property: When output tokens are 0, effective tokens should equal input tokens
     */
    it("should return input tokens when output tokens are 0", () => {
      fc.assert(
        fc.property(fc.nat({ max: 1000000 }), (inputTokens) => {
          const { calculateEffectiveTokens } =
            require("@/services/bedrock-service") as typeof import("@/services/bedrock-service");

          const effectiveTokens = calculateEffectiveTokens(inputTokens, 0);

          // Property: with 0 output, effective = input
          expect(effectiveTokens).toBe(inputTokens);
        }),
        { numRuns: 100 }
      );
    });
